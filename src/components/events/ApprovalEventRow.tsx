...
interface ApprovalEventRowProps {
  event: EventContext;
  tokenName: string;
}
...
export const ApprovalEventRow: React.FC<ApprovalEventRowProps> = ({
  event,
  tokenName,
}) => {
  const [open, setOpen] = React.useState(false);

  const owner = event.parameters.find(p => p.name === 'owner')?.value;
  const spender = event.parameters.find(p => p.name === 'spender')?.value;
  const value = event.parameters.find(p => p.name === 'value')?.value;

  if (!owner || !spender || !value) {
    return null;
  }
      ...
      >
        ...
        >
          <Stack alignItems="flex-end" sx={{ minWidth: 120 }}>
            <Tooltip title={owner}>
              <Typography variant="body2">{shortenAddress(owner)}</Typography>
            </Tooltip>
          ...
          <Stack alignItems="flex-start" sx={{ minWidth: 120 }}>
            <Tooltip title={spender}>
              <Typography variant="body2">{shortenAddress(spender)}</Typography>
            </Tooltip>
          </Stack>
        </Stack>
      </Stack>
